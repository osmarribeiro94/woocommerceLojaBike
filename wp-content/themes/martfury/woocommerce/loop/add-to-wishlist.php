...
echo apply_filters(
	...
	sprintf(
		'<a href="%s" data-quantity="%s" data-product_id="%d" data-variations="%s" class="%s" aria-label="%s" data-rel="tooltip" title="%s">
			%s
		...
		esc_attr( isset( $args['aria-label'] ) ? $args['aria-label'] :sprintf( __( 'Add %s to the wishlist', 'martfury' ), '&ldquo;' . $product->get_title() . '&rdquo;' ) ),
		esc_html__( 'Wishlist', 'martfury' ),
		empty( $args['icon'] ) ? '' : '<span class="wcboost-wishlist-button__icon">' . $args['icon'] . '</span>',
		esc_html( isset( $args['label'] ) ? $args['label'] : __( 'Add to wishlist', 'martfury' ) )
	),
	$args
);

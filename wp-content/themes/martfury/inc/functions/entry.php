...
if ( ! function_exists( 'martfury_get_breadcrumbs' ) ) :
	function martfury_get_breadcrumbs() {
		...
		if ( ! $breadcrumbs_enabled && class_exists('WPSEO_Options') && method_exists('WPSEO_Options', 'get') ) {
...
if ( ! function_exists( 'martfury_is_catalog' ) ) :
	function martfury_is_catalog() {
		...
		if ( function_exists( 'is_shop' ) && ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() || is_tax() ) ) {
			return true;
		}
	...
	if ( intval( martfury_get_option( 'lazyload' ) ) ) {
		...
		if ( $image ) {
			...
			if ( $attributes ) {
				...
				$output          = sprintf(
					'<img src="%s" data-original="%s"  alt="%s" class="lazy %s" data-src="%s" data-large_image="%s" data-large_image_width="%s" data-large_image_height="%s">',
					esc_url( $image_trans ),
					esc_url( $image[0] ),
					esc_attr( $alt ),
					esc_attr( $css_class ),
...
if ( ! function_exists( 'martfury_get_page_base_url' ) ) :
	function martfury_get_page_base_url() {
		...
		return apply_filters( 'martfury_get_page_base_url', $link );
	}
endif;
...
if ( ! function_exists( 'martfury_recently_viewed_products' ) ) :
	function martfury_recently_viewed_products( $atts ) {
		...
		if ( isset( $atts['title'] ) && $atts['title'] ) {
			$output[] = sprintf( '<h4 class="title">%s</h4>', esc_html( $atts['title'] ) );
		}
		...
		if ( empty( $viewed_products ) ) {
			...
			foreach ( $viewed_products as $product_id ) {
				...
				$output[] = sprintf(
					'<li>' .
					'<a href="%s">%s</a>' .
					'<div class="product-info"><h6 class="product-title">%s</h6><div class="product-price">%s</div></div>'.
					'</li>',
	...
	function martfury_currency_switcher( $show_desc = false ) {
		...
		if ( class_exists( 'WOOCS' ) ) {
			...
			foreach ( $currencies as $key => $currency ) {
				$currency_value = empty($currency[ $key_cur ]) ? $currency['name'] : $currency[ $key_cur ];
			...
			$currency_value = empty($currencies[ $WOOCS->current_currency ][ $key_cur ]) ? $currencies[ $WOOCS->current_currency ][ 'name' ] : $currencies[ $WOOCS->current_currency ][ $key_cur ];
			$currency_dd = sprintf(
			...
			foreach ( $function_currencies as $currency_code ) {
				if ( isset( $currencies[ $currency_code ] ) ) {
					$the_text = function_exists('alg_format_currency_switcher') ? alg_format_currency_switcher( $currencies[ $currency_code ], $currency_code, false ) : '';
 ...
 */
if ( ! function_exists( 'martfury_body_open' ) ) {
	function martfury_body_open() {
 ...
 * @return string
 */
function martfury_sanitize_svg( $svg ) {
	$allowed   = array();
	$whitelist = array(
		'a'              => array(
			'class',
